...
exports.initialize_mongodb = function (next, server) {
    ...
    if ( ! server.notes.mongodb ) {
        ...
        if (plugin.cfg.mongodb.auth_source) {
            connectionString += `?authSource=${plugin.cfg.mongodb.auth_source}`;
        }
...
exports.data_post_email = function (next, connection) {
    ...
    // Get messageid
    let _mid = connection.transaction.header.headers_decoded ? connection.transaction.header.headers_decoded['message-id'][0] : new ObjectID() + '@haraka-helpmonks.com';
    _mid = _mid.replace(/<|>/g, '');
...
exports.bounced_email = function (next, hmail, error) {
    ...
    // Vars
    const plugin = this;
    const _rcpt = hmail.todo.rcpt_to[0];
    const _rcpt_to = _rcpt.original.slice(1, -1);
    const _date = moment().subtract(1, 'h').toISOString();
    const _query = { 'rcpt_to' : _rcpt_to, 'timestamp' : { '$gt' : new Date(_date) } };
...
function _storeAttachments (connection, plugin, attachments, mail_object, cb) {
    ...
    async.each(attachments, function (attachment, each_callback) {
        ...
        if (attachment.generatedFileName && attachment.generatedFileName.length > 200) {
            // Split up filename
            const _filename_new = attachment.generatedFileName.split('.');
            // Get extension
            const _fileExt = _filename_new.pop();
            // Get filename
            const _filename_pop = _filename_new.pop();
            // Just in case filename is longer than 200 chars we make sure to take from the left
            const _filename_200 = S(_filename_pop).left(200).s;
            // Add it together
            const _final = _filename_200 + '.' + _fileExt;
        ...
        const attachment_directory = path.join(attachments_folder_path, attachment_checksum);
        ...
        fs.mkdirp(attachment_directory, function (error, result) {
            ...
            const attachment_full_path = path.join(attachment_directory, attachment.generatedFileName);
            // Log
            ...
            fs.writeFile(attachment_full_path, attachment.content, function (error) {
                ...
                if (attachment.generatedFileName.toLowerCase() === 'winmail.dat') {
                    ...
                    const exec_command = `tnef ${attachment_full_path} -C ${attachment_directory}`;

                    plugin.lognotice('converting winmail.dat by calling :', exec_command);

                    // execute the tnef process to extract the real attachment
                    const tnef_process = exec(exec_command, function (error, stdout, stderr) {
                        const general_error = stderr || error;
                        ...
                        fs.readdir(attachment_directory, function (error, contents) {
                            ...
                            async.eachLimit(contents.filter((fn) => fn !== 'winmail.dat'), 3, function (file_name, each_callback) {
                                ...
                                const attachment_file_path = path.join(attachment_directory, file_name);
